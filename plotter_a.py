...
def oven_plot(d):
    'Oven log plot, input date: yyyy_mm_dd'
    
    ###read csv file, creates pandas DataFrame###
    data_raw = pd.read_csv(os.getcwd()+'/Data/' + str(d) + '.txt',header=None,names=['t','pressure','chamber_top','air_top','ion_pump','ion_gauge','foil_top','heater_r','valve','out_cross'])
    ...
    ax[0].plot(ts,data_raw['ion_pump'])
    ax[0].plot(ts,data_raw['ion_gauge'])
